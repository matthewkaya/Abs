...
@router.post("/request")
async def beta_request(body: BetaRequestBody) -> dict:
    # Honeypot: silently 200 to bots so they don't retry.
    if body.website:
        logger.info("[beta] honeypot triggered email=%s", body.email)
        return {"ok": True, "queued": False}

    if _has_recent_request(body.email):
        raise HTTPException(429, "duplicate_recent_request")
    ...
    return {"ok": True, "auto_approved": False, "request_id": req.id, "status": "pending"}
